...
import { createAzureClients, powerVmWithProgress, startVm } from '$lib/server/azure';
	...
	try {
		...
		if (wantsProgressStream(event.request)) {
			return vmOperationStream({
				...
				run: async (progress) => {
					await progress(
						operationProgressEvent('power-on-auth', 'success', 'Azure 账号已连接，准备开机', {
							resourceGroup,
							vmName
						})
					);
