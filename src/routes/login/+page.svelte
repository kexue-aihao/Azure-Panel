...
<div class="flex min-h-screen items-center justify-center p-4">
	<div class="card w-full max-w-md space-y-5 p-6">
		...
		<form class="space-y-3" onsubmit={(e) => { e.preventDefault(); login(); }}>
		...
		<form class="space-y-3 border-t border-border pt-4" onsubmit={(e) => { e.preventDefault(); register(); }}>
			<p class="text-sm text-muted">没有账号？注册</p>
			<input class="input" bind:value={registerEmail} type="email" placeholder="注册邮箱" required />
			<input class="input" bind:value={registerPassword} type="password" placeholder="至少 6 位密码" required />
